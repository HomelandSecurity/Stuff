...
import os
print("Checking if local files exist:")
for split, path in {
    ...
    print(f"{split}: exists={os.path.exists(path)}, size={os.path.getsize(path) if os.path.exists(path) else 'N/A'}")
# load_dataset
#wikitext = load_from_disk('/fred/oz413/LLM/wikitext/default')
...
#ADDED BY IBRAHIM DELEAT IF ISSUES CAUSE----------
from transformers import AutoTokenizer, AutoModel

cache_dir = "/fred/oz413/hf_cache"
...
#IBRAHIM HERE AGAIN COMMENT IS ORIGINAL UNDER IT LOOP IS NEW
